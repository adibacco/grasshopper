...
void grasshopper_main()
 {
  unsigned long long int  mkey[4], * * subkeys, plaintext[2], ciphertext[2];
  unsigned char i;

  /* Allocation of subkey array      */
  subkeys = (unsigned long long int * *)calloc(10, sizeof(unsigned long long int *));
  for(i = 0;i < 10;i++) subkeys[i] = (unsigned long long int *)calloc(2, sizeof(unsigned long long int));
